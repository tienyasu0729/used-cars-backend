...
public class StaffService {
	...
	/**
	 * Nhân viên active, role SalesStaff/BranchManager, đang gắn chi nhánh (StaffAssignment hoặc manager chi nhánh).
	 */
	@Transactional(readOnly = true)
	public void assertActiveStaffLinkedToBranch(long staffUserId, int branchId) {
		User u = userRepository.findActiveByIdWithRoles(staffUserId)
				.orElseThrow(() -> new BusinessException(ErrorCode.STAFF_NOT_FOUND, "Không tìm thấy nhân viên."));
		if (!hasStaffRole(u)) {
			throw new BusinessException(ErrorCode.STAFF_NOT_FOUND, "Người được chọn không phải nhân viên chi nhánh.");
		}
		if (!isUserLinkedToBranch(staffUserId, branchId)) {
			throw new BusinessException(ErrorCode.STAFF_NOT_IN_BRANCH, "Nhân viên không thuộc chi nhánh của lịch hẹn.");
		}
	}
